...
#ifndef MENDER_DEVICE_H
#define MENDER_DEVICE_H

#include <mender/error.h>

struct mender_device;

mender_err_t mender_device_reboot(struct mender_device *dev);
mender_err_t mender_device_swap_partitions(struct mender_device *dev);

mender_err_t mender_device_install_update_start(struct mender_device *dev, uint64_t size);
